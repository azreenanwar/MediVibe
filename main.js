...
function isRegisterPage() {
  const p = (location.pathname || "").toLowerCase();
  return p.endsWith("/register.html") || p.endsWith("register.html");
}
...
function requireAuth() {
  ...
  if (!session) {
    const back = encodeURIComponent(location.pathname || "dashboard.html");
...
function hydrateNav() {
  const session = AUTH.getSession();
  // SHOW/HIDE semua benda yang ada class showIfLogin
  document.querySelectorAll(".showIfLogin").forEach(el => {
    el.style.display = session ? "inline-flex" : "none";
  });
...
function bindAuthForms() {
  ...
  if (loginForm) {
    loginForm.addEventListener("submit", (e) => {
      ...
      const safeNext = next ? stripLeadingSlash(next) : "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const s = AUTH.getSession();
  if ((isLoginPage() || isRegisterPage()) && s) {
